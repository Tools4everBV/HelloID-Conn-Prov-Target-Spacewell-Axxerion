...
if (-not($dryRun -eq $true)) {
    try {
        Write-Verbose "Deleting Axxerion user '$($p.DisplayName)'"
        $clobMBValue = @{
        ...
        if ($disableResponse.response -eq 'none'){
            ...
            $success = $true
            $auditLogs.Add([PSCustomObject]@{
                Message = "Account for '$($p.DisplayName)' successfully deleted with id: '$($aRef)'"
                IsError = $false
            })
        }
    } catch {
        ...
        if ( $($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or $($ex.Exception.GetType().FullName -eq 'System.Net.WebException')) {
            ...
            $auditMessage = "Could not delete user account for '$($p.DisplayName)', Error $($errorObject.ErrorMessage)"
        } else {
            $auditMessage = "Could not delete user account for '$($p.DisplayName)', Error: $($ex.Exception.Message)"
        }
        $auditLogs.Add([PSCustomObject]@{
            Action  = "DeleteAccount"
